...
With VI at ~53×, **Kokoro EN on 5 CPU workers (~27× aggregate) is the fresh-tranche long pole** — consider `EN_WORKERS` 7–8 at the next tranche boundary (worker-count changes require `merge_worker_manifests` + deleting stale worker CSVs first). Remaining VI ideas, in value order: per-voice prefix-KV prefill cache; codec-stage work (now ~28%).
