...
VI ~53× on MPS, EN ~26× aggregate on CPU — **EN remains the fresh-tranche long pole but is at the machine's bandwidth ceiling**. Remaining VI ideas, in value order: per-voice prefix-KV prefill cache; codec-stage work (now ~28%).
