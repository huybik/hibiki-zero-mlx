...
- **Layout**: 7 all-CoreML EN workers saturate the GPU at **~70–75× RT aggregate** (16.4×/worker warm; near-linear to 4 workers). Don't mix in compiled-CPU workers: rows shard uniformly, so 2.5× CPU workers drag the wall while CoreML workers idle. The compiled-CPU path remains behind `EN_COREML_WORKERS < EN_WORKERS`.
...
VI ~53× on MPS, EN **~70× aggregate on Core ML/Metal GPU** (2026-07-30 pass above; CPU-compiled path retired to fallback) — **VI is now the fresh-tranche long pole**. Remaining VI ideas, in value order: per-voice prefix-KV prefill cache; codec-stage work (now ~28%). Remaining EN idea if ever needed: batch chunks through one predict (the masked-norm design already supports per-row masks) to amortize the ~250 ms/chunk CoreML dispatch overhead.
