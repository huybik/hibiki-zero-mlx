...
The only supported training path is five full-data epochs of direct
...
- Train exactly 719,120 frozen rows per epoch for five epochs / 224,725 steps. Validation is
  138 eligible rows, batch 4, observed maximum 277 frames, and `shuffle=False`.
- Recovery artifacts live only under `grounded_v2_full_direct_voice_5epoch`.

There is no pilot, ASR warm-start or replay, post-source curriculum,
contrastive or anti-repetition loss, shuffled validation, or six-epoch
schedule.
...
recovery_commit="$(curl -fsSL \
  https://huggingface.co/huybik/hibiki-zero-vi-full-sft/resolve/main/grounded_v2_full_direct_voice_5epoch/run.json \
...
Use this only when the remote `grounded_v2_full_direct_voice_5epoch/` prefix is empty:
...
`finetune/runs/vi_grounded_v2_full_direct_voice_5epoch`, uploads `run.json`, trains five
epochs, validates and saves every 9,000 steps plus step 224,725, promotes the
lowest validation loss, and keeps two recovery pairs plus the best model.
...
prefix=grounded_v2_full_direct_voice_5epoch
./.venv/bin/hf download "$HIBIKI_HF_REPO" \
  ...
  "$prefix/best/best_stepBBBBBB.safetensors" \
  "$prefix/best/best_stepBBBBBB.json" \
...
cp "$restore_dir/$prefix/best/best_stepBBBBBB.safetensors" "$run_dir/"
cp "$restore_dir/$prefix/best/best_stepBBBBBB.json" "$run_dir/best.json"
...
identity, the current promoted best model/marker after step 9,000, and the commit
recorded by `run.json`. It cannot extend the five-epoch
...
./.venv/bin/python finetune/eval.py \
  ...
  --checkpoint finetune/runs/vi_grounded_v2_full_direct_voice_5epoch/model_stepNNNNNN.safetensors \
  --pairs finetune/pairs/val128.jsonl \
  --limit 128 --batch-size 8 --text-temp 0.4 \
  --stop-on-eos --text-only --seed 42 \
  --out-dir finetune/runs/vi_grounded_v2_full_direct_voice_5epoch/eval_stepNNNNNN
```
