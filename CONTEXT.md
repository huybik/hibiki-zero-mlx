...
- `code/mlx_hibiki_patch.py` — the same four deltas as a **runtime monkey-patch of stock `moshi_mlx`**. The local fork-based scripts no longer import it; it is kept only as the portable shim published to HF for users on the PyPI `moshi-mlx` package.
