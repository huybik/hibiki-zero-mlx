...
- `finetune/cache_phomt_stream.py` builds the published PhoMT cache directly
  ...
  CTC backpointers on-device, release each batch, and bound concurrent workers by
  row and audio-sample budgets. `finetune/publish_grounded_cache.py` validates and checksum-publishes
